...
import { ArrowLeft, Trash2, Clock, Pencil, Search, ChevronLeft, ChevronRight } from 'lucide-react';
import { getAllClientDrafts, deleteClientDraft, ClientDraft, ClientDraftPaginationParams, PaginatedClientDraftResponse } from '../../services/api';
...
export function ClientDrafts() {
  ...
  const fetchDrafts = useCallback(async () => {
    try {
      ...
      const effectiveGlobalSearch = globalSearch.length >= 3 ? globalSearch : '';
      const effectiveCompanyNameFilter = companyNameFilter.length >= 3 ? companyNameFilter : '';
      const effectiveShortCodeFilter = shortCodeFilter.length >= 3 ? shortCodeFilter : '';
      const effectiveListNameFilter = listNameFilter.length >= 3 ? listNameFilter : '';
      const effectiveContactPersonFilter = contactPersonFilter.length >= 3 ? contactPersonFilter : '';
      ...
      const response: PaginatedClientDraftResponse = await getAllClientDrafts(params);
    ...
    companyNameFilter.length >= 3 || companyNameFilter === '' ? companyNameFilter : 'inactive',
    shortCodeFilter.length >= 3 || shortCodeFilter === '' ? shortCodeFilter : 'inactive',
    listNameFilter.length >= 3 || listNameFilter === '' ? listNameFilter : 'inactive',
    contactPersonFilter.length >= 3 || contactPersonFilter === '' ? contactPersonFilter : 'inactive',
    dateFilter,
    createdDateFilter
  ]);
  ...
  useEffect(() => {
    ...
    companyNameFilter.length >= 3 || companyNameFilter === '' ? companyNameFilter : null,
    shortCodeFilter.length >= 3 || shortCodeFilter === '' ? shortCodeFilter : null,
    listNameFilter.length >= 3 || listNameFilter === '' ? listNameFilter : null,
    contactPersonFilter.length >= 3 || contactPersonFilter === '' ? contactPersonFilter : null,
    dateFilter,
    createdDateFilter
  ]);
          ...
          <div className="pagination-controls top">
            <div className="pagination-info">
              <span className="pagination-text">
                Showing {Math.min((pagination.page - 1) * pagination.limit + 1, pagination.totalFiltered)} to{' '}
                {Math.min(pagination.page * pagination.limit, pagination.totalFiltered)} of {pagination.totalFiltered} entries
