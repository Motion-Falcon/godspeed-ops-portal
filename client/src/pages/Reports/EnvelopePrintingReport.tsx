...
const getTableColumns = (t: (key: string) => string): { key: keyof EnvelopePrintingReportRow; label: string; format?: (val: unknown, row?: EnvelopePrintingReportRow) => string }[] => [
  { key: 'city', label: t('reports.columns.city') },
  { key: 'list_name', label: t('reports.columns.listName') },
  { key: 'week_ending', label: t('reports.columns.weekEnding'), format: (val) => formatWeekDate(String(val ?? '')) },
  ...
  { key: 'total_amount', label: t('reports.columns.totalAmount'), format: (val) => val !== undefined && val !== 'N/A' ? `$${val}` : String(val ?? '') },
  { key: 'tax_rate', label: t('reports.columns.taxRate'), format: (val) => val !== undefined && val !== 'N/A' ? `${val}%` : String(val ?? '') },
  { key: 'hst_gst', label: t('reports.columns.hstGst'), format: (val) => val !== undefined && val !== 'N/A' ? `$${val}` : String(val ?? '') },
...
const getCsvColumns = (tableColumns: ReturnType<typeof getTableColumns>) => [...tableColumns];
