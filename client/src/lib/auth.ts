...
export async function getJobseekerVerificationStatus(
...
export async function getUsersManager(
  userId: string
): Promise<AllAuthUserListItem | null> {
...
// Demo login function - bypasses normal login flow for demo users
// Note: All demo users must have the same password: 'Demo@123'
// If you need to change this, update the DEMO_PASSWORD constant below
export const demoLogin = async (email: string) => {
  // Demo password for all demo accounts
  // IMPORTANT: Ensure all demo users (admin@email.com, recruiter@email.com, jobseeker@email.com)
  // have this password set in Supabase
  const DEMO_PASSWORD = "Demo@123";

  try {
    // Use the server-side login endpoint instead of direct Supabase call
    // This avoids database schema query issues
    const { loginUserAPI } = await import("../services/api/auth");
    const result = await loginUserAPI(email, DEMO_PASSWORD);
