-- Git record: find_matching_candidates (Supabase RPC) — OLD VERSION
-- Used by: GET /api/jobseekers/position-candidates/:positionId
-- This version marks candidates as unavailable (is_available = FALSE) when they have
-- an active/upcoming assignment in the same date range; with only_available=true
-- those candidates are excluded from results. Superseded by find_matching_candidates.sql
-- to allow assigning the same job seeker to multiple positions in the same time range.
...
BEGIN
    ...
    WITH available_candidates AS (
        SELECT 
            ...
            jp.availability,
            jp.bio_embedding,
            CASE WHEN pca.candidate_id IS NULL THEN TRUE ELSE FALSE END AS is_available
