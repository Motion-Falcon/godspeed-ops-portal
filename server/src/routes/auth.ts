...
router.post("/send-verification", async (req, res) => {
  try {
    ...
    if (!isTwilioConfigured) {
      ...
      return res
        .status(500)
        .json({ error: "Phone verification service is not available" });
    }
...
router.post("/verify-otp", async (req, res) => {
  try {
    let { phoneNumber, code, userId } = req.body as {
      phoneNumber: string;
      code: string;
      userId?: string;
    };
  ...
  activityLogger({
    onSuccess: (req, res) => {
      ...
      return {
        ...
        secondaryEntityName:
          res.locals.newUser?.user_metadata?.name || req.body.name || "Unknown",
        displayMessage: `New user registered: ${
...
router.post("/validate-credentials", async (req, res) => {
  try {
    ...
    const isRecruiterOrAdmin =
      ...
      email.includes("@godspeed"); // Add your admin email domain(s) here
...
router.post("/update-password", async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ error: "Authentication token is required for password reset" });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Password update failed with user token" });
    }
  } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const phoneNumberRaw = (req.body as any).phoneNumber as
        | string
        | undefined;
...
router.get("/check-phone", async (req, res) => {
  try {
    ...
    const { data: userId, error } = await supabase.rpc("get_user_id_by_phone", {
      user_phone: phone,
    });
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to check phone availability" });
    }
