...
const aiVerificationUrl =
  process.env.AI_VERIFICATION_URL ||
  "https://ai-verification-ff5a17fb5c4a.herokuapp.com/analyze-profile-documents";
...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error(
    "CRITICAL: Missing Supabase URL or Service Key for jobseekers route."
  );
...
function formatName(
  profile: DbJobseekerProfile | DbJobseekerProfileListView
): string {
  return (
...
router.get("/", isAdminOrRecruiter, async (req, res) => {
  try {
    ...
    let baseQuery = supabaseAdmin.from("jobseeker_profiles").select(`
        id, 
        user_id, 
        first_name, 
        last_name, 
    ...
    if (countError) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to get total count of profiles" });
    }
    ...
    if (filteredCountError) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to get filtered count of profiles" });
    }
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch jobseeker profiles from database" });
    }
    ...
    res.status(500).json({
      error: "An unexpected error occurred while fetching jobseeker profiles",
    });
  }
});
...
function applyFilters(
  ...
  if (search && search.trim().length > 0) {
    ...
    query = query.or(
      `verification_status.ilike.%${searchTerm}%,first_name.ilike.%${searchTerm}%,last_name.ilike.%${searchTerm}%,email.ilike.%${searchTerm}%,mobile.ilike.%${searchTerm}%,experience.ilike.%${searchTerm}%,employee_id.ilike.%${searchTerm}%,city.ilike.%${searchTerm}%,province.ilike.%${searchTerm}%`
    );
  }
  ...
  if (nameFilter && nameFilter.trim().length > 0) {
    ...
    query = query.or(
      `first_name.ilike.%${nameTerm}%,last_name.ilike.%${nameTerm}%`
    );
  }
  ...
  if (
    ...
    experienceFilter.trim().length > 0
  ) {
...
router.get("/profile/:id", async (req, res) => {
  try {
    ...
    if (profileByIdError) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch detailed jobseeker profile" });
    }
    ...
    if (profile.documents && profile.documents.length > 0) {
      ...
      formattedProfile.documents = profile.documents.map(
        (doc: DocumentRecord) => ({
          ...doc,
          aiValidation: null,
        })
      ...
      if (documentIds.length > 0) {
        ...
        if (aiError) {
          ...
          formattedProfile.documents = formattedProfile.documents.map(
            (doc: DocumentRecord) => {
    ...
    if (profile.updated_by_user_id) {
      try {
        const { data: updaterData, error: updaterError } =
          await supabaseAdmin.auth.admin.getUserById(
    ...
    console.error(
      "Unexpected error fetching detailed jobseeker profile:",
      error
    );
    res.status(500).json({
      error:
        "An unexpected error occurred while fetching detailed jobseeker profile",
    });
  }
});
  ...
  activityLogger({
    onSuccess: (req, res) => {
      ...
      let actionType:
        | "verify_jobseeker"
        | "reject_jobseeker"
        | "pending_jobseeker" = "pending_jobseeker";
      ...
      return {
        ...
        } jobseeker profile for ${profileName}${
          status === "rejected" && rejectionReason
            ? ` with reason: ${rejectionReason}`
            : ""
        }${
          status === "verified" && updatedProfile.employee_id
            ? ` and assigned employee code ${updatedProfile.employee_id}`
            : ""
        }`,
        category: "candidate_management" as const,
        priority:
          status === "rejected" ? ("high" as const) : ("normal" as const),
        status: "completed",
        metadata: {
    ...
    try {
      ...
      if (status === "verified" && !currentProfile.employee_id) {
        try {
          ...
          if (codeError) {
            ...
            console.log(
              `Generated employee code: ${employeeCode} for profile ${id}`
            );
          }
      ...
      if (error) {
        ...
        if (error.code === "PGRST116") {
          return res
            .status(404)
            .json({ error: "Profile not found to update status" });
        }
        return res
          .status(500)
      ...
      if (!data) {
        // Should be redundant due to error handling
        return res
          .status(404)
          .json({ error: "Profile not found after update attempt" });
      }
      ...
      const formattedProfile: JobSeekerDetailedProfile = {
        ...
        bio:
          updatedDbProfile.license_type ||
          updatedDbProfile.work_preference ||
          undefined,
        // Add type to doc in find callback
        resume:
          updatedDbProfile.documents?.find(
            (doc: DocumentRecord) => doc.documentType === "resume"
      ...
      res.status(500).json({
        error: "An unexpected error occurred while updating profile status",
      });
    }
  }
);
  ...
  activityLogger({
    onSuccess: (req, res) => {
      ...
      return {
        ...
        metadata: {
          profileId: req.params.id,
          updatedFields: Object.keys(profileData).filter(
            (key) => key !== "documents"
          ),
          hasDocuments: !!(
  ...
  async (req, res) => {
    try {
      ...
      if (fetchError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to verify profile existence" });
      }
      ...
      if (profileData.email && profileData.email !== existingProfile.email) {
        ...
        if (emailCheckError) {
          ...
          return res
            .status(500)
            .json({ error: "Failed to validate email uniqueness" });
        }
        ...
        profileData.documents = profileData.documents.map((doc: any) => {
          ...
          if (doc.id && existingDocsMap.has(doc.id)) {
            ...
            if (hasMetadataChanged) {
              ...
              if (doc.documentPath !== oldDoc.documentPath)
                changedFields.push("path");
              if (doc.documentType !== oldDoc.documentType)
                changedFields.push("type");
              if (doc.documentTitle !== oldDoc.documentTitle)
                changedFields.push("title");
              if (doc.documentFileName !== oldDoc.documentFileName)
                changedFields.push("fileName");
              if (doc.documentNotes !== oldDoc.documentNotes)
                changedFields.push("notes");

              console.log(
                `Document metadata changed - Old ID: ${
                  doc.id
                }, New ID: ${newId}, Changed fields: ${changedFields.join(
                  ", "
                )}`
              );
      ...
      if (profileData.billingEmail !== undefined)
        updateData.billing_email = profileData.billingEmail || null;
      if (profileData.mobile) updateData.mobile = profileData.mobile;
      if (profileData.licenseNumber)
        updateData.license_number = profileData.licenseNumber;
      if (profileData.passportNumber)
        updateData.passport_number = profileData.passportNumber;
      ...
      if (profileData.workPermitUci)
        updateData.work_permit_uci = profileData.workPermitUci;
      if (profileData.workPermitExpiry)
        updateData.work_permit_expiry = profileData.workPermitExpiry;
      if (profileData.businessNumber)
        updateData.business_number = profileData.businessNumber;
      if (profileData.corporationName)
        updateData.corporation_name = profileData.corporationName;
      ...
      if (profileData.workPreference)
        updateData.work_preference = profileData.workPreference;
      if (profileData.bio) updateData.bio = profileData.bio;
      if (profileData.licenseType)
        updateData.license_type = profileData.licenseType;
      if (profileData.experience)
        updateData.experience = profileData.experience;
      if (profileData.manualDriving)
        updateData.manual_driving = profileData.manualDriving;
      if (profileData.availability)
        updateData.availability = profileData.availability;
      if (profileData.weekendAvailability !== undefined)
        updateData.weekend_availability = profileData.weekendAvailability;
      if (profileData.payrateType)
        updateData.payrate_type = profileData.payrateType;
      ...
      if (profileData.paymentMethod)
        updateData.payment_method = profileData.paymentMethod;
      ...
      if (profileData.overtimeHours)
        updateData.overtime_hours = profileData.overtimeHours;
      if (profileData.overtimeBillRate)
        updateData.overtime_bill_rate = profileData.overtimeBillRate;
      if (profileData.overtimePayRate)
        updateData.overtime_pay_rate = profileData.overtimePayRate;
      ...
      // Send profile data to AI verification service asynchronously (fire-and-forget)
      // This runs in the background and doesn't block the response
      (async () => {
        try {
          console.log(
            `Sending updated profile data to AI verification service at: ${aiVerificationUrl}`
          );
          ...
          if (verificationResponse.ok) {
            ...
            console.log(
              "AI verification service response:",
              verificationResult
            );
          } else {
            console.error(
              "AI verification service error:",
          ...
          console.error(
            "Error sending data to AI verification service:",
            verificationError
          );
        }
      })();

      // Return response immediately without waiting for AI verification
      res.json({
    ...
    onSuccess: (req, res) => {
      ...
      return {
        ...
        primaryEntityName: jobseekerProfile
          ? `${jobseekerProfile.first_name || ""} ${
              jobseekerProfile.last_name || ""
            }`.trim() ||
            jobseekerProfile.email ||
            `Jobseeker ${req.params.id}`
          : `Jobseeker ${req.params.id}`,
        displayMessage: `Deleted jobseeker profile ${
          jobseekerProfile
            ? `for ${jobseekerProfile.first_name || ""} ${
                jobseekerProfile.last_name || ""
              }`.trim() || jobseekerProfile.email
            : req.params.id
        }`,
        category: "candidate_management",
        priority: "normal",
  ...
  async (req, res) => {
    try {
      ...
      if (fetchError && fetchError.code !== "PGRST116") {
        ...
        return res
          .status(500)
          .json({ error: "Failed to fetch jobseeker profile" });
      }
      ...
      if (deleteError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to delete jobseeker profile" });
      }
      ...
      try {
        ...
        const { data: userData, error: userDataError } =
          await supabaseAdmin.auth.admin.getUserById(jobseekerProfile.user_id);
        ...
        if (!userDataError && userData?.user) {
          ...
          if (metadataError) {
            console.error(
              "Error updating user metadata to remove hasProfile flag:",
              metadataError
            );
          } else {
            console.log(
              `Successfully updated hasProfile flag to false for user ${jobseekerProfile.user_id}`
            );
          }
        } else {
          console.error(
            "Error fetching user data for updating metadata:",
            userDataError
          );
        }
      } catch (metadataError) {
      ...
      res.status(500).json({
        error: "An unexpected error occurred while deleting the profile",
      });
    }
  }
);
...
router.get("/drafts", isAdminOrRecruiter, async (req, res) => {
  try {
    ...
    const createdDateFilter = (
      (req.query.createdDateFilter as string) || ""
    ).trim();
    ...
    if (filteredCountError) {
      console.error(
        "Error getting filtered count of drafts:",
        filteredCountError
      );
      return res
        .status(500)
    ...
    const creatorIds = [
      ...new Set(
        drafts.map((draft: any) => draft.created_by_user_id).filter(Boolean)
      ),
    ];
    const updaterIds = [
      ...new Set(
        drafts.map((draft: any) => draft.updated_by_user_id).filter(Boolean)
      ),
    ];
    ...
    if (creatorFilter) {
      formattedDrafts = formattedDrafts.filter((draft) => {
        ...
        return (
          (creator.name && creator.name.toLowerCase().includes(searchTerm)) ||
    ...
    if (updaterFilter) {
      formattedDrafts = formattedDrafts.filter((draft) => {
        ...
        return (
          (updater.name && updater.name.toLowerCase().includes(searchTerm)) ||
 ...
 */
router.get(
  "/position-candidates/:positionId",
  isAdminOrRecruiter,
  async (req, res) => {
    try {
      ...
      const { data: candidatesData, error: candidatesError } =
        await supabaseAdmin.rpc("find_matching_candidates", {
      ...
      if (candidatesError) {
        console.error(
          "Error calling get_position_candidates function:",
          candidatesError
        );
      ...
      let formattedCandidates: FormattedPositionCandidate[] =
        candidatesData.map((candidate: PositionCandidateResult) => ({
          ...
          similarityScore: parseFloat(
            candidate.similarity_score.toString() || "0"
          ),
          ...
          (candidate) =>
            ...
            (candidate.experience &&
              candidate.experience
                ...
                .includes(search.toLowerCase())) ||
            (candidate.bio &&
              candidate.bio.toLowerCase().includes(search.toLowerCase()))
        );
      }
      ...
      const paginatedCandidates = formattedCandidates.slice(
        offset,
        offset + limitNum
      );
      ...
      res.status(500).json({
        error:
          "An unexpected error occurred while fetching position candidates",
    ...
    return res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Internal server error checking email",
    });
  }
});
