...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const checkNumbers = [nextInvoiceNumber, `INV-${nextInvoiceNumber}`];
      ...
      for (const checkNumber of checkNumbers) {
        ...
        if (existingError) {
          console.error(
            "Error checking existing invoice number:",
            existingError
          );
          return res
            .status(500)
            .json({ error: "Failed to validate invoice number" });
        }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (countError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to get total count of invoices" });
      }

      // Get filtered count
      let filteredCountQuery = supabase.from("invoices").select(
        `
          *,
    ...
    onSuccess: async (req: Request, res: Response) => {
      ...
      if (clientId) {
        try {
          ...
          if (client) {
            clientName =
              client.company_name || client.short_code || "Unknown Client";
          }
        } catch (error) {
      ...
      const finalGrandTotal =
        newInvoice?.grand_total ?? invoiceData.grandTotal ?? 0;
      const finalCurrency =
        newInvoice?.currency ?? invoiceData.currency ?? "CAD";
      ...
      return {
        ...
        displayMessage: `Created invoice ${
          newInvoice?.invoice_number
        } for ${clientName} - $${finalGrandTotal.toFixed(2)} ${finalCurrency}`,
        category: "financial",
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const field of requiredFields) {
        if (
          invoiceData[field as keyof InvoiceData] === undefined ||
          invoiceData[field as keyof InvoiceData] === null
        ) {
          return res
            .status(400)
...
router.put(
  ...
  activityLogger({
    onSuccess: async (req: Request, res: Response) => {
      ...
      const finalGrandTotal =
        updatedInvoice?.grand_total ?? invoiceData.grandTotal ?? 0;
      const finalCurrency =
        updatedInvoice?.currency ?? invoiceData.currency ?? "CAD";
      const finalStatus =
        updatedInvoice?.status ?? invoiceData.status ?? "draft";
      ...
      return {
        ...
        secondaryEntityId: clientId || null,
        secondaryEntityName: clientName,
        displayMessage: `Updated invoice ${
        ...
        metadata: {
          ...
          grandTotal: finalGrandTotal,
          totalHours:
            updatedInvoice?.total_hours ?? invoiceData.totalHours ?? 0,
        },
      };
    },
  }),
  async (req: Request, res: Response) => {
    try {
      ...
      let existingQuery = supabase.from("invoices").select("*").eq("id", id);
      ...
      if (userType === "jobseeker" && existingInvoice.status !== "draft") {
        return res
          .status(403)
          .json({ error: "You can only update draft invoices" });
      }
      ...
      if (invoiceData.invoiceDate)
        updateData.invoice_date = invoiceData.invoiceDate;
      ...
      if (invoiceData.subtotal !== undefined)
        updateData.subtotal = invoiceData.subtotal;
      if (invoiceData.totalTax !== undefined)
        updateData.total_tax = invoiceData.totalTax;
      if (invoiceData.totalHst !== undefined)
        updateData.total_hst = invoiceData.totalHst;
      if (invoiceData.totalGst !== undefined)
        updateData.total_gst = invoiceData.totalGst;
      if (invoiceData.totalQst !== undefined)
        updateData.total_qst = invoiceData.totalQst;
      if (invoiceData.grandTotal !== undefined)
        updateData.grand_total = invoiceData.grandTotal;
      if (invoiceData.totalHours !== undefined)
        updateData.total_hours = invoiceData.totalHours;
      if (invoiceData.emailSent !== undefined)
        updateData.email_sent = invoiceData.emailSent;
      if (invoiceData.emailSentDate)
        updateData.email_sent_date = invoiceData.emailSentDate;
      if (invoiceData.invoice_sent_to)
        updateData.invoice_sent_to = invoiceData.invoice_sent_to;
      if (invoiceData.documentGenerated !== undefined)
        updateData.document_generated = invoiceData.documentGenerated;
      if (invoiceData.documentPath)
        updateData.document_path = invoiceData.documentPath;
      if (invoiceData.documentFileName)
        updateData.document_file_name = invoiceData.documentFileName;
      if (invoiceData.documentFileSize)
        updateData.document_file_size = invoiceData.documentFileSize;
      if (invoiceData.documentMimeType)
        updateData.document_mime_type = invoiceData.documentMimeType;
      if (invoiceData.documentGeneratedAt)
        updateData.document_generated_at = invoiceData.documentGeneratedAt;
      if (invoiceData.invoiceData)
        updateData.invoice_data = invoiceData.invoiceData;
      ...
      const isVersionedUpdate = Object.keys(invoiceData).some((key) =>
        versionedFields.includes(key)
      );
      if (isVersionedUpdate) {
        const prevVersion = existingInvoice.version || 1;
        const newVersion = prevVersion + 1;
        updateData.version = newVersion;
        const prevHistory = Array.isArray(existingInvoice.version_history)
          ? existingInvoice.version_history
          : [];
        updateData.version_history = [
          ...prevHistory,
  ...
  activityLogger({
    onSuccess: async (req: Request, res: Response) => {
      ...
      return {
        ...
        secondaryEntityName: clientName,
        displayMessage: `Deleted invoice ${
          deletedInvoice?.invoice_number
        } for ${clientName} - $${(deletedInvoice?.grand_total ?? 0).toFixed(
          2
        )} ${deletedInvoice?.currency || "CAD"}`,
        category: "financial",
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (updateError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to update invoice document" });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const formattedTimesheets = (timesheets || []).map((timesheet: any) => ({
        ...
        premiumPayRate: parseFloat(timesheet.premium_pay_rate) || 0,
  ...
  activityLogger({
    onSuccess: (req, res) => {
      ...
      return {
        ...
        displayMessage: `Sent invoice ${
          result.invoiceNumber || result.invoice_number || id
        } to ${email}`,
        category: "financial",
        priority: "normal",
        metadata: {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (client.invoice_cc3 && client.email_address3)
        cc.push(client.email_address3);
      if (client.invoice_cc_dispatch && client.dispatch_dept_email)
        cc.push(client.dispatch_dept_email);
      if (client.invoice_cc_accounts && client.accounts_dept_email)
        cc.push(client.accounts_dept_email);

      // Download PDF from Supabase Storage
      if (!invoice.document_path || !invoice.document_file_name) {
        return res
          .status(400)
          .json({ error: "Invoice PDF not found for this invoice" });
      }
      ...
      if (pdfError || !pdfData)
        return res
          .status(500)
          .json({ error: "Failed to download invoice PDF" });
      // Read as buffer
          ...
          if (
            ...
            try {
              ...
              if (fileError || !fileData) {
                console.error(
                  "[SendGrid] Failed to download attachment:",
                  att.fileName,
                  fileError
                );
                continue;
              ...
              console.error(
                "[SendGrid] Error downloading attachment:",
                att.fileName,
                err
              );
            }
      ...
      console.log("[SendGrid] Sending email with payload:", {
        ...
        cc: sendPayload.cc,
        attachments: sendPayload.attachments.map((a) => ({
          filename: a.filename,
          type: a.type,
      ...
      return res
        .status(200)
        .json({ success: true, message: "Invoice email sent successfully" });
    } catch (error) {
