...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { data: existingProfileByEmail, error: emailCheckError } =
        await supabase
      ...
      try {
        ...
        if (userLookupError) {
          ...
          const { data: userData, error: userDataError } =
            await supabase.auth.admin.getUserById(profileUserId);
          ...
          const generateSecurePassword = () => {
            ...
            for (let i = 0; i < remainingLength; i++) {
              remainingChars += characters.charAt(
            ...
            const unshuffled =
              uppercaseLetter + lowercaseLetter + number + remainingChars;
      ...
      try {
        ...
        const { error: metadataError } =
          await supabase.auth.admin.updateUserById(profileUserId, {
            user_metadata: mergedMetadata,
          });

        if (metadataError) {
          console.error(
            "Error updating user metadata with hasProfile flag:",
            metadataError
          );
        } else {
          console.log(
            `Successfully updated hasProfile flag for user ${profileUserId}`
          );
        }
      } catch (metadataError) {
      ...
      (async () => {
        try {
          console.log(
            `Sending profile data to AI verification service at: ${aiVerificationUrl}`
          );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (lookupError) {
        console.error(
          "Error checking email availability in profiles:",
          lookupError
        );
        return res
          .status(500)
      ...
      if (draftLookupError) {
        console.error(
          "Error checking email availability in drafts:",
          draftLookupError
        );
        return res
          .status(500)
          .json({ error: "Failed to check email availability in drafts" });
      }
